...
> 版本：v1.1 | 作者：许清楚（Xu）| 日期：2026-06-07
...
| D2 | GeoGebra 布局 | 集成在对话页面中（非独立页面），点击"发送到 GeoGebra"按钮时将函数渲染到已初始化的 GeoGebra 区域并跳转 |
...
| D11 | GeoGebra 跳转交互 | 点击按钮后跳转到集成了 GeoGebra 的页面区域，函数自动渲染 |
...
| P0-4 | GeoGebra 交互界面 | 集成在对话页面中（非独立页面），支持函数图像绘制与交互操作 |
| P0-5 | 函数发送到 GeoGebra | 从对话界面将函数表达式传递给同页 GeoGebra 区域并自动绘图 |
| P0-6 | 历史对话持久化 | 前端本地存储（localStorage / uni.setStorageSync），支持导出 JSON 备份 |
...
GeoGebra 作为对话页面的一个区域嵌入，而非独立页面。用户点击"发送到 GeoGebra"按钮时，函数渲染到已初始化的 GeoGebra 区域并自动跳转（滚动）到该区域。

具体实现路径：

1. **页面结构**：对话页面上方为对话区域，下方为 GeoGebra WebView 区域
2. **GeoGebra 加载**：使用 WebView 组件加载本地 GeoGebra 中间页（中间页内嵌 GeoGebra JavaScript API）
3. **通信机制**：
   - H5 端：通过 `postMessage` 直接通信
   - 小程序端：通过 URL 参数 + `evalJS` 向 WebView 注入命令
4. **函数渲染**：点击"发送到 GeoGebra"后，将函数表达式传入 WebView，调用 `evalCommand("f(x) = ...")` 绘制函数图像
5. **跳转交互**：渲染完成后自动滚动页面至 GeoGebra 区域，用户可交互操作
...
```
对话界面 → [点击 📈 按钮] → 提取函数表达式
  → 传入同页 GeoGebra WebView → GeoGebra API 绘图
  → 页面自动滚动至 GeoGebra 区域
```

- 用户在对话界面点击"发送到 GeoGebra"按钮
- 前端提取当前函数表达式（如 `y = 1/x`）
- 通过 WebView 通信机制将函数表达式传入 GeoGebra
- GeoGebra 接收后调用 `evalCommand` 绘制函数图像
- 页面自动滚动至 GeoGebra 区域，用户可直接交互操作
...
│                                 │
│     ┌─────────────────────┐    │
│     │   GeoGebra 区域      │    │  ← GeoGebra WebView（同页嵌入）
│     │   （函数图像渲染）    │    │
│     └─────────────────────┘    │
│                                 │
└─────────────────────────────────┘
...
- **底部输入区**：文本输入框 + 发送按钮
- **GeoGebra 区域**：对话页面下方嵌入 GeoGebra WebView，函数渲染后自动滚动至此
...
- 点击后提取【函数表达式】中的函数公式
- 传入同页 GeoGebra WebView 区域，调用 `evalCommand` 绘制函数图像
- 绘制完成后页面自动滚动至 GeoGebra 区域
...
flowchart TD
    ...
    R --> T[显示总结面板]
    T --> L
    S --> U[传入同页 GeoGebra WebView]
    U --> V[GeoGebra API 绘制函数]
    V --> W[页面滚动至 GeoGebra 区域]
    W --> X[用户交互操作]
...
| `VITE_MAX_CHAT_COUNT` | 最大对话保存数量 | `50` |
...
| 编号 | 问题 | 影响范围 | 建议 |
|------|------|----------|------|
| Q1 | 历史对话存储上限触发清理时的交互策略？ | P0-6 | 需确认是自动清理最旧对话，还是弹窗让用户手动选择 |
| Q2 | 多函数叠加绘图的交互方式？ | P2-3 | 需确认是在同一对话中累积绘制，还是每次绘制替换上一次 |
| Q3 | 解题过程导出格式优先级？ | P2-2 | 图片优先还是 PDF 优先，影响实现方案选择 |
