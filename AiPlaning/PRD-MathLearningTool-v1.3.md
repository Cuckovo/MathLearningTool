...
> 版本：v1.3 | 作者：许清楚（Xu）| 日期：2026-06-07
...
| D2 | GeoGebra 布局 | 底部 Tab 栏切换模式：底部 Tab 栏包含「Geo工具」和「AI对话」两个 Tab，GeoGebra 界面位于「Geo工具」Tab，对话界面位于「AI对话」Tab；点击 📈 按钮时切换到「Geo工具」Tab 进行函数渲染 |
...
| D8 | 主题色调 | 使用 assets 目录下的 CSS 变量文件管理主题色，主色调 #4F46E5 等色彩变量定义在 `assets/` 下的 CSS 文件中，而非硬编码 |
...
| D11 | GeoGebra Tab 切换交互 | 点击 📈 按钮后提取函数表达式，通过 switchTab 切换到「Geo工具」Tab，函数自动渲染 |
...
| D17 | 底部 Tab 栏结构 | 底部 Tab 栏包含 2 个 Tab：「Geo工具」和「AI对话」。Geo工具 Tab 为已初始化的 GeoGebra 界面，AI对话 Tab 为对话主界面。导航方式使用 UniApp 的 switchTab，而非 navigateTo |
...
| 编号 | 需求 | 说明 |
|------|------|------|
| P0-1 | AI 对话解题界面 | 核心对话 UI（AI对话 Tab），支持用户输入数学问题、展示 AI 回复 |
| P0-2 | 系统提示词设计 | AI 同时输出：解题过程 + 是否可绘图判断 + 函数表达式 |
| P0-3 | LaTeX 公式渲染 | 使用 KaTeX + WebView 对话中正确解析并渲染 LaTeX 数学公式 |
| P0-4 | GeoGebra 交互界面 | GeoGebra 界面位于底部 Tab 栏的「Geo工具」Tab，支持函数图像绘制与交互操作 |
| P0-5 | 函数发送到 GeoGebra | 从 AI对话 Tab 提取函数表达式，通过 switchTab 切换到「Geo工具」Tab 并自动绘图 |
...
| P0-8 | 底部 Tab 栏导航 | 底部 Tab 栏包含「Geo工具」和「AI对话」两个 Tab，使用 switchTab 切换 |
...
| P2-3 | 深色模式 | 适配深色模式 |
| P2-4 | 主题色配置 | 主题色通过 assets 目录下的 CSS 变量文件管理，支持通过修改 CSS 变量切换主题色调 |
...
**确认方案：底部 Tab 栏 — Geo工具 Tab**

GeoGebra 界面作为底部 Tab 栏的「Geo工具」Tab 存在，与「AI对话」Tab 并列。用户在 AI对话 Tab 中产出的函数公式，点击"发送到 GeoGebra"按钮后，提取函数表达式并通过 switchTab 切换到「Geo工具」Tab 进行渲染。

具体实现路径：

1. **页面结构**：底部 Tab 栏双 Tab 结构 — 「Geo工具」Tab + 「AI对话」Tab（D17）
2. **Tab 导航**：使用 UniApp 的 `switchTab` 进行 Tab 切换，而非 `navigateTo` 页面跳转
3. **GeoGebra 加载**：在「Geo工具」Tab 中使用 WebView 组件加载本地 GeoGebra 中间页（中间页内嵌 GeoGebra JavaScript API）
4. **通信机制**：
   - H5 端：通过全局状态（如 Vuex/Pinia 或 uni.$emit/uni.$on）传递函数表达式，切换 Tab 后 GeoGebra 页面读取并绘图
   - 小程序端：通过全局状态 + `evalJS` 向 WebView 注入命令
5. **函数渲染**：点击"发送到 GeoGebra"后，将函数表达式写入全局状态，switchTab 切换到「Geo工具」Tab，GeoGebra 页面检测到待渲染函数后调用 `evalCommand("f(x) = ...")` 绘制函数图像
6. **Tab 切换交互**：切换到「Geo工具」Tab 后函数自动渲染，用户可交互操作；点击「AI对话」Tab 回到对话界面
...
- 导航方式为 switchTab（Tab 切换），不是 navigateTo（页面跳转）
...
```
AI对话 Tab → [点击 📈 按钮] → 提取函数表达式
  → 写入全局状态 → switchTab 切换到「Geo工具」Tab
  → GeoGebra 检测待渲染函数 → API 绘制函数图像
  → 用户交互操作 → 点击「AI对话」Tab 回到对话界面
```

- 用户在 AI对话 Tab 中点击"发送到 GeoGebra"按钮
- 前端提取当前函数表达式（如 `y = 1/x`），写入全局状态
- 通过 switchTab 切换到「Geo工具」Tab
- Geo工具 Tab 中的 GeoGebra WebView 检测到待渲染函数，调用 `evalCommand` 绘制函数图像
- 用户在 Geo工具 Tab 交互操作，点击「AI对话」Tab 回到对话界面
...
│                                 │
│     ┌─────────────────────┐    │
│     │   GeoGebra 区域      │    │  ← GeoGebra WebView（全屏展示）
│     │   （函数图像渲染）    │    │
│     │                     │    │
│     │                     │    │
│     └─────────────────────┘    │
...
- **底部输入区**：文本输入框 + 发送按钮
- **底部 Tab 栏**：2 个 Tab —「Geo工具」和「AI对话」，当前活跃 Tab 带●标记
...
- 点击后提取【函数表达式】中的函数公式，写入全局状态
- 通过 switchTab 切换到「Geo工具」Tab，GeoGebra 检测待渲染函数后调用 `evalCommand` 绘制函数图像
- 用户在「Geo工具」Tab 交互操作，点击「AI对话」Tab 回到对话界面
...
flowchart TD
    ...
    R --> T[显示总结面板]
    T --> L
    S --> U[写入全局状态 + switchTab 切换到 Geo工具 Tab]
    U --> V[GeoGebra API 绘制函数]
    V --> W[用户交互操作]
    W --> X[点击 AI对话 Tab 回到对话界面]
    ...
    C -->|点击 Geo工具 Tab| BB[切换到 Geo工具 Tab]
    BB --> CC[GeoGebra 界面]
```
...
主题色使用 `assets/` 目录下的 CSS 变量文件管理，而非硬编码到组件中。

**方案说明**：

| 维度 | 方案 |
|------|------|
| 管理方式 | CSS 自定义属性（CSS Variables） |
| 文件位置 | `assets/styles/theme.css`（或类似路径） |
| 主色调 | `#4F46E5`（Indigo） |
| 变量命名 | `--color-primary`、`--color-primary-light`、`--color-primary-dark` 等 |
...
**使用规则**：
- 所有组件中的主题色必须引用 CSS 变量，不得硬编码颜色值
- 深色模式（P2-3）可通过覆盖 CSS 变量实现
- 主题色配置（P2-4）通过修改 CSS 变量文件切换
...
> **注**：主题色不再通过 .env 配置，改由 `assets/styles/theme.css` 中的 CSS 变量管理（见第7章）。
