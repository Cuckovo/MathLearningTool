<template>
  <view class="page-switcher">
    <view class="switcher-track">
      <view
        class="switcher-option"
        :class="{ active: activeView === 'geo' }"
        @tap="switchTo('geo')"
      >
        <svg class="switcher-icon" viewBox="70 8 40 33" width="16" height="16">
          <!-- 函数曲线 -->
          <path class="icon-curve" fill-rule="evenodd" clip-rule="evenodd" d="M100.3 17.2C98.8001 15.2 96.3001 14 93.1001 13.8C93.2001 13.5 93.2001 13.2 93.2001 12.9C93.2001 12.3 93.0001 11.7 92.8001 11.2C97.1001 11.3 100.8 13.2 102.8 16.2C101.8 16.2 101 16.6 100.3 17.2ZM86.4001 15.4C85.8001 14.7 85.4001 13.8 85.4001 12.9V12.8C82.6001 14.1 80.2001 16.2 78.4001 18.7C79.3001 19 80.1001 19.7 80.6001 20.5C80.7001 20.3 80.8001 20.1 81.0001 19.9C82.2001 18.2 83.9001 16.7 86.0001 15.6C86.0619 15.5382 86.1237 15.5146 86.2091 15.482C86.2619 15.4618 86.3237 15.4382 86.4001 15.4ZM78.6001 26.1C78.2001 26.2 77.7001 26.3 77.2001 26.3C76.7001 26.3 76.2001 26.2 75.8001 26C75.7001 27.8 75.9001 29.5 76.8001 31.4C77.3001 32.5 78.0001 33.4 78.8001 34.2C79.2001 33.3 79.8001 32.6 80.7001 32.2C80.2001 31.6 79.8001 31 79.4001 30.3C78.8001 29 78.5001 27.5 78.6001 26.1ZM86.7001 35.1C86.6001 35.1 86.5251 35.075 86.4501 35.05C86.3751 35.025 86.3001 35 86.2001 35C86.2001 35.0899 86.2203 35.1798 86.2425 35.2788C86.2698 35.4 86.3001 35.5348 86.3001 35.7C86.3001 36.4 86.1001 37.1 85.7001 37.7C88.5001 38.1 91.7001 37.7 94.7001 36.4C94.1001 35.8 93.7001 34.9 93.6001 34C91.3001 35 88.8001 35.4 86.7001 35.1ZM101.9 23.8C102.2 23.9 102.5 24 102.9 24C103.5 24 104.1 23.8 104.6 23.6C104.4 25.7 103.6 28.2 101.9 30.6C101.75 30.8 101.575 31 101.4 31.2C101.225 31.4 101.05 31.6 100.9 31.8C100.5 31 99.8001 30.3 98.9001 30C99.0001 29.9 99.1001 29.775 99.2001 29.65C99.3001 29.525 99.4001 29.4 99.5001 29.3C100.9 27.4 101.7 25.5 101.9 23.8Z" fill="#666666"/>
          <!-- 数据点外环 -->
          <path class="icon-ring" fill-rule="evenodd" clip-rule="evenodd" d="M73.3 22.4C73.3 20.2 75 18.5 77.2 18.5C79.4 18.5 81.1 20.2 81.1 22.4C81.1 24.6 79.4 26.3 77.2 26.3C75 26.3 73.3 24.6 73.3 22.4ZM74.5 22.4C74.5 23.9 75.7 25.1 77.2 25.1C78.7 25.1 79.9 23.9 79.9 22.4C79.9 20.9 78.7 19.7 77.2 19.7C75.7 19.7 74.5 20.9 74.5 22.4Z" fill="#191919"/>
          <path class="icon-ring" fill-rule="evenodd" clip-rule="evenodd" d="M78.5001 35.7C78.5001 33.5 80.2001 31.8 82.4001 31.8C84.6001 31.8 86.3001 33.5 86.3001 35.7C86.3001 37.9 84.6001 39.6 82.4001 39.6C80.2001 39.6 78.5001 37.9 78.5001 35.7ZM79.7001 35.7C79.7001 37.2 80.9001 38.4 82.4001 38.4C83.9001 38.4 85.1001 37.2 85.1001 35.7C85.1001 34.2 83.9001 33 82.4001 33C80.9001 33 79.7001 34.2 79.7001 35.7Z" fill="#191919"/>
          <path class="icon-ring" fill-rule="evenodd" clip-rule="evenodd" d="M85.4 12.9C85.4 10.7 87.1 9 89.3 9C91.5 9 93.2 10.7 93.2 12.9C93.2 15.1 91.5 16.8 89.3 16.8C87.1 16.8 85.4 15.1 85.4 12.9ZM86.6 12.9C86.6 14.4 87.8 15.6 89.3 15.6C90.8 15.6 92 14.4 92 12.9C92 11.4 90.8 10.2 89.3 10.2C87.8 10.2 86.6 11.4 86.6 12.9Z" fill="#191919"/>
          <path class="icon-ring" fill-rule="evenodd" clip-rule="evenodd" d="M99.0001 20.1C99.0001 17.9 100.7 16.2 102.9 16.2C105.1 16.2 106.8 17.9 106.8 20.1C106.8 22.3 105.1 24 102.9 24C100.7 24 99.0001 22.3 99.0001 20.1ZM100.2 20.1C100.2 21.6 101.4 22.8 102.9 22.8C104.4 22.8 105.6 21.6 105.6 20.1C105.6 18.6 104.4 17.4 102.9 17.4C101.4 17.4 100.2 18.6 100.2 20.1Z" fill="#191919"/>
          <path class="icon-ring" fill-rule="evenodd" clip-rule="evenodd" d="M93.6 33.6C93.6 31.4 95.3 29.7 97.5 29.7C99.7 29.7 101.4 31.4 101.4 33.6C101.4 35.8 99.7 37.5 97.5 37.5C95.3 37.5 93.6 35.8 93.6 33.6ZM94.8 33.6C94.8 35.1 96 36.3 97.5 36.3C99 36.3 100.2 35.1 100.2 33.6C100.2 32.1 99 30.9 97.5 30.9C96 30.9 94.8 32.1 94.8 33.6Z" fill="#191919"/>
          <!-- 数据点实心圆 -->
          <circle class="icon-dot" cx="77.2001" cy="22.4" r="2.7" fill="#9999FF"/>
          <circle class="icon-dot" cx="82.4001" cy="35.7" r="2.7" fill="#9999FF"/>
          <circle class="icon-dot" cx="89.3" cy="12.9" r="2.7" fill="#9999FF"/>
          <circle class="icon-dot" cx="102.9" cy="20.1" r="2.7" fill="#9999FF"/>
          <circle class="icon-dot" cx="97.5" cy="33.6" r="2.7" fill="#9999FF"/>
        </svg>
        <text class="switcher-label">GeoGebra</text>
      </view>
      <view
        class="switcher-option"
        :class="{ active: activeView === 'chat' }"
        @tap="switchTo('chat')"
      >
        <svg class="switcher-icon" viewBox="0 0 34 24" width="16" height="16" fill="currentColor">
          <path d="M33.615 2.598c-.36-.176-.515.16-.726.33-.072.055-.132.127-.193.193-.526.562-1.14.93-1.943.887-1.174-.067-2.176.302-3.062 1.2-.188-1.107-.814-1.767-1.766-2.191-.498-.22-1.002-.441-1.35-.92-.244-.341-.31-.721-.433-1.096-.077-.226-.154-.457-.415-.496-.282-.044-.393.193-.504.391-.443.81-.614 1.702-.598 2.605.04 2.033.898 3.652 2.603 4.803.193.132.243.264.182.457-.116.397-.254.782-.376 1.179-.078.253-.194.308-.465.198-.936-.391-1.744-.97-2.458-1.669-1.213-1.173-2.31-2.467-3.676-3.48a16.254 16.254 0 0 0-.975-.668c-1.395-1.354.183-2.467.548-2.599.382-.138.133-.612-1.102-.606-1.234.005-2.364.42-3.803.97a4.34 4.34 0 0 1-.66.193 13.577 13.577 0 0 0-4.08-.143c-2.667.297-4.799 1.558-6.365 3.712C.116 8.436-.327 11.378.215 14.444c.57 3.233 2.22 5.91 4.755 8.002 2.63 2.17 5.658 3.233 9.113 3.03 2.098-.122 4.434-.403 7.07-2.633.664.33 1.362.463 2.518.562.892.083 1.75-.044 2.414-.182 1.04-.22.97-1.184.593-1.36-3.05-1.421-2.38-.843-2.99-1.311 1.55-1.834 3.918-5.093 4.648-9.531.072-.49.164-1.18.153-1.577-.006-.242.05-.336.326-.364a5.903 5.903 0 0 0 2.187-.672c1.977-1.08 2.774-2.853 2.962-4.978.028-.325-.006-.661-.35-.832ZM16.39 21.73c-2.956-2.324-4.39-3.089-4.982-3.056-.554.033-.454.667-.332 1.08.127.407.293.688.526 1.046.16.237.271.59-.161.854-.952.589-2.607-.198-2.685-.237-1.927-1.134-3.537-2.632-4.673-4.68-1.096-1.972-1.733-4.087-1.838-6.345-.028-.545.133-.738.676-.837A6.643 6.643 0 0 1 5.086 9.5c3.017.441 5.586 1.79 7.74 3.927 1.229 1.217 2.159 2.671 3.116 4.092 1.02 1.509 2.115 2.946 3.51 4.125.494.413.887.727 1.263.958-1.135.127-3.028.154-4.324-.87v-.002Zm1.417-9.114a.434.434 0 0 1 .587-.408c.06.022.117.055.16.105a.426.426 0 0 1 .122.303.434.434 0 0 1-.437.435.43.43 0 0 1-.432-.435Zm4.402 2.257c-.283.116-.565.215-.836.226-.421.022-.88-.149-1.13-.358-.387-.325-.664-.506-.78-1.073-.05-.242-.022-.617.022-.832.1-.463-.011-.76-.338-1.03-.265-.22-.603-.28-.974-.28a.8.8 0 0 1-.36-.11c-.155-.078-.283-.27-.161-.508.039-.077.227-.264.271-.297.504-.286 1.085-.193 1.623.022.498.204.875.578 1.417 1.107.553.639.653.815.968 1.295.25.374.476.76.632 1.2.094.275-.028.5-.354.638Z"/>
        </svg>
        <text class="switcher-label">对话</text>
      </view>
    </view>
  </view>
</template>

<script setup lang="ts">
import { computed } from 'vue'
import { useGeogebraStore } from '../stores/geogebra_store'

const geogebraStore = useGeogebraStore()
const activeView = computed(() => geogebraStore.activeView)

/** 切换视图（不卸载页面，v-show 控制显隐） */
function switchTo(view: 'chat' | 'geo'): void {
  geogebraStore.switchView(view)
}
</script>

<style scoped>
.page-switcher {
  display: flex;
  align-items: center;
  justify-content: center;
}

/* 椭圆胶囊容器 */
.switcher-track {
  display: flex;
  align-items: center;
  background: var(--color-bg-page, #f2f2f2);
  border-radius: 999px;
  padding: 3px;
  gap: 2px;
}

/* 单个选项 */
.switcher-option {
  display: flex;
  align-items: center;
  justify-content: center;
  height: 32px;
  padding: 0 18px;
  border-radius: 999px;
  cursor: pointer;
  transition: all 0.25s ease;
}

/* 选中状态：品牌绿色背景 + 白色文字 */
.switcher-option.active {
  background: var(--color-brand, #90c208);
  box-shadow: 0 1px 3px rgba(144, 194, 8, 0.25);
}

.switcher-option.active .switcher-label {
  color: #fff;
  font-weight: 600;
}

/* 未选中（灰度）状态 */
.switcher-option:not(.active) .switcher-label {
  color: var(--color-text-tertiary, rgba(34, 34, 34, 0.5));
  font-weight: 400;
}

/* 选项图标 */
.switcher-icon {
  width: 16px;
  height: 16px;
  flex-shrink: 0;
  margin-right: 6px;
}

/* 对话图标：单色，直接 currentColor 继承 */
.switcher-icon use {
  pointer-events: none;
}

/* 选中态 —— 聊天图标 */
.switcher-option.active .switcher-icon {
  color: #fff;
}

/* 未选中态 —— 聊天图标 */
.switcher-option:not(.active) .switcher-icon {
  color: var(--color-text-tertiary, rgba(34, 34, 34, 0.5));
}

/* ── GeoGebra 图标：多色，激活态保留原始配色 ── */

/* 未选中（灰度）：曲线浅灰 */
.switcher-option:not(.active) .switcher-icon .icon-curve {
  fill: #a0a0a0;
}

/* 未选中（灰度）：外环中灰 */
.switcher-option:not(.active) .switcher-icon .icon-ring {
  fill: #c0c0c0;
}

/* 未选中（灰度）：数据点淡灰 */
.switcher-option:not(.active) .switcher-icon .icon-dot {
  fill: #d5d5d5;
}

.switcher-label {
  font-size: 14px;
  white-space: nowrap;
  line-height: 1;
}
</style>
