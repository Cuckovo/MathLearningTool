...
function extractSolvingProcess(content: string): string {
  console.log(`${LOG_PREFIX} extractSolvingProcess：输入长度=${content.length}`)
  const match = content.match(/【解题过程】\s*([\s\S]*?)(?=【图像判断】|$)/)
  const result = match ? match[1].trim() : content.trim()
  console.log(`${LOG_PREFIX} extractSolvingProcess：结果长度=${result.length}`)
  return result
}

/** 提取【图像判断】内容 */
function extractImageJudgment(content: string): string {
  console.log(`${LOG_PREFIX} extractImageJudgment：输入长度=${content.length}`)
  const match = content.match(/【图像判断】\s*([\s\S]*?)(?=【函数表达式】|$)/)
  const result = match ? match[1].trim() : ''
  console.log(`${LOG_PREFIX} extractImageJudgment：结果=`, result)
  return result
}
...
function latexToGeogebraExpr(latex: string): string {
  console.log(`${LOG_PREFIX} latexToGeogebraExpr：输入=`, latex)
  let expr = latex.trim()

  // 1. 处理 \frac{a}{b} → (a)/(b)
  // 需要递归处理嵌套
  expr = expr.replace(/\\frac\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/g, '($1)/($2)')
  console.log(`${LOG_PREFIX} frac 处理后：`, expr)

  // 2. 处理 \sqrt{a} → sqrt(a), \sqrt[n]{a} → nroot(n, a)
  expr = expr.replace(/\\sqrt\[([^\]]+)\]\{([^}]+)\}/g, 'nroot($1, $2)')
  expr = expr.replace(/\\sqrt\{([^}]+)\}/g, 'sqrt($1)')
  console.log(`${LOG_PREFIX} sqrt 处理后：`, expr)
  ...
  console.log(`${LOG_PREFIX} 三角函数/对数处理后：`, expr)

  // 4. 去掉下标的 _{} 标记（GeoGebra 用下标表示序列，不需要）
  expr = expr.replace(/_\{[^}]+\}/g, '')
  console.log(`${LOG_PREFIX} 下标移除后：`, expr)

  // 5. 去掉 ^ 的上标花括号（保留 ^ 但去花括号如果只有单个字符）
  expr = expr.replace(/\^\{([^{}]+)\}/g, '^($1)')
  console.log(`${LOG_PREFIX} 上标处理后：`, expr)
  ...
  console.log(`${LOG_PREFIX} 反斜杠命令清理后：`, expr)
  ...
  console.log(`${LOG_PREFIX} sin x → sin(x) 处理后：`, expr)

  // 7. 处理 \{ \} → 普通括号（LaTeX 转义花括号）
  expr = expr.replace(/\\\{/g, '{').replace(/\\\}/g, '}')
  console.log(`${LOG_PREFIX} 花括号转义处理后：`, expr)

  // 8. 清理多余花括号（平衡配对）
  expr = expr.replace(/^\{(.+)\}$/, '$1')
  console.log(`${LOG_PREFIX} 花括号清理后：`, expr)

  // 9. 清理空格、换行
  expr = expr.replace(/\s+/g, ' ').trim()
  console.log(`${LOG_PREFIX} latexToGeogebraExpr：最终结果=`, expr)
...
function stripChineseDescription(rawExpr: string): string {
  ...
  if (doubleDollarMatch) {
    ...
    console.log(`${LOG_PREFIX} 双美元符号剥离后：`, expr)
  }
  ...
  if (singleDollarMatch) {
    ...
    if (captured.length > 1 && captured.endsWith('$')) {
      expr = captured
      console.log(`${LOG_PREFIX} 单美元符号剥离后：`, expr)
    }
  }
  ...
  expr = expr.replace(/\u201c.*$/, '') // "
  expr = expr.replace(/\u300c.*$/, '') // 「
  console.log(`${LOG_PREFIX} 中文说明剥离后：`, expr)
...
function extractFunctionExpression(content: string): string {
  console.log(`${LOG_PREFIX} extractFunctionExpression：输入长度=${content.length}`)
  const match = content.match(/【函数表达式】\s*([\s\S]*?)$/)
  if (!match) {
    console.log(`${LOG_PREFIX} extractFunctionExpression：未找到匹配`)
    return ''
  }

  let expr = match[1].trim()
  console.log(`${LOG_PREFIX} extractFunctionExpression：原始表达式=`, expr)

  // 去除 "无" 等不可绘制标记
  if (expr === '无' || expr === '' || expr.toLowerCase() === 'none') {
    console.log(`${LOG_PREFIX} extractFunctionExpression：表达式为空或"无"`)
    return ''
  }
  ...
  console.log(`${LOG_PREFIX} 美元符号移除后：`, expr)

  // ⭐ 新增：去除中文说明和描述文本
  expr = stripChineseDescription(expr)
  console.log(`${LOG_PREFIX} stripChineseDescription 处理后：`, expr)

  // 去除 "y = " / "y=" 前缀
  expr = expr.replace(/^y\s*=\s*/i, '')
  console.log(`${LOG_PREFIX} "y=" 前缀移除后：`, expr)

  // 转为 GeoGebra 可用格式
  expr = latexToGeogebraExpr(expr)
  console.log(`${LOG_PREFIX} extractFunctionExpression：最终结果=`, expr)
...
export function parse(rawContent: string): ParsedAiResponse {
  console.log(`${LOG_PREFIX} parse：开始解析，输入长度=${rawContent.length}`)
  try {
    ...
    console.log(`${LOG_PREFIX} parse：结果=`, { solvingProcessLength: solvingProcess.length, imageJudgment, functionExpr, canPlot })
